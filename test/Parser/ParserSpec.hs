...
spec = do
    ...
    describe "identifier" $ do
        ...
        specify "does not parse a string starting with a digit" $
            forAll ((:) <$> digitCharGen <*> listOf (oneof [idCharGen, digitCharGen])) $ \s -> isLeft $ parse identifier "" s
        specify "does not parse a string starting with a not-allowed special character" $
